...
def transform_to_row_vector(x):
    # transform x[i] example to a row vector
    num_examples = list(range(x.shape[0]))
    x_ravel = np.vstack([x[i].ravel() for i in num_examples])
    return x_ravel
...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    x_ravel = transform_to_row_vector(x)    
    out = x_ravel.dot(w) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        # normalize the data by using running_mean and running_var from trainning step
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    # get needed variables for backpropagration
    gamma, x, mean, var, eps, xhat = cache
    m = dout.shape[0]
    
    # initialize d% variables
    dx = np.zeros_like(x)
    dgamma = np.zeros_like(gamma)
    dbeta = np.zeros_like(gamma) # gamma and beta have the same shape
    
    # compute immediate gradients
    # follow the formula in the original paper
    d_xhat = dout * gamma
    
    d_ivar = d_xhat * (x - mean)
    d_var = (-0.5) * np.sum(d_ivar, axis = 0) * \
             np.power(var + eps, -1.5)
    
    d_mean1 = -np.sum(d_xhat, axis = 0) / np.sqrt(var + eps)
    d_mean21 = d_var
    d_mean22 = np.sum(-2.0 * (x - mean), axis = 0) / m
    d_mean2 = d_mean21 * d_mean22    
    d_mean = d_mean1 + d_mean2    
    
    # compute final gradients
    dx1 = d_xhat / np.sqrt(var + eps) 
    dx2 = d_var * 2.0 * (x - mean) / m
    dx3 = d_mean / m
    dx =  dx1 + dx2 + dx3    
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ###########################################################################
    # compute mean and var of each data point
...
def layernorm_backward(dout, cache): # not finished, to be completed later
    """
    ...
    # tranpose needed variables to column vector
    ...
    d_ivar = d_xhat * (x - mean)
    d_var = (-0.5) * np.sum(d_ivar, axis = 1, keepdims = True) * \
             np.power(var + eps, -1.5)
    
    d_mean1 = -np.sum(d_xhat, axis = 1, keepdims = True) / np.sqrt(var + eps)
    d_mean21 = d_var
    d_mean22 = np.sum(-2.0 * (x - mean), axis = 1, keepdims = True) / m
    d_mean2 = d_mean21 * d_mean22    
    d_mean = d_mean1 + d_mean2    
...
def conv_backward_naive(dout, cache):
    ...
    dx = np.zeros_like(x_pad) # we will remove padded elements at the end
    dw = np.zeros_like(w)
    db = np.zeros_like(b)
    ...
    # go through each training example:    
    for n in range(N):
        # compute convolutional grad at f-th layer
        for f in range(F):
            # compute each point at out[n, f, :, :]
            ...
            for h_prime in range(H_prime):
                for w_prime in range(W_prime):
                       ...
                       dw[f] += dout[n, f, h_prime, w_prime] * x_mask
    ...
    # remove padded elements from dx
    dx = dx[:, :, pad : -1 - pad + 1, pad : -1 - pad + 1]    
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def max_pool_forward_naive(x, pool_param):
    ...
    # compute size of the out matrix
        ...
        for c in range(C):
            for h_prime in range(H_prime):
                for w_prime in range(W_prime):
                    ...
                    out[n, c, h_prime, w_prime] = np.amax(x_mask)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, pool_param)
        ...
        for c in range(C):
            for h_prime in range(H_prime):
                for w_prime in range(W_prime):
                    ...
                    # calculate index of max element in x
                    max_idx = np.argmax(x_mask)
                    h, w = divmod(max_idx, pool_width) 
                    
                    dx[n, c, (h_prime * stride) + h, (w_prime * stride) + w] = \
